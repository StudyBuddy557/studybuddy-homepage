...
export default function SalesBot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-4 font-sans hidden md:flex">
      {isOpen && (
        <div className="w-[350px] overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 animate-in slide-in-from-bottom-10 fade-in duration-300 origin-bottom-right">
          {/* Header */}
          <div className="flex items-center gap-4 bg-gradient-to-r from-[#20B2AA] to-[#18968F] p-5 text-white">
            <div className="relative shrink-0 w-12 h-12">
              <Image 
                ...
                alt="AI Tutor" 
                fill
                className="rounded-full border-2 border-white/30 bg-white object-cover" 
              />
      ...
      {!isOpen && (
        <button 
          ...
          className="group relative flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12)] ring-1 ring-slate-900/5 transition-all hover:scale-110 hover:shadow-[0_8px_40px_rgba(32,178,170,0.3)] overflow-hidden"
        >
          <Image 
            src="/StudyBuddy_AI_tutor_Avatar.png" 
            alt="Chat" 
