export interface WikiTerm {
  slug: string;
  term: string;
  category: 'Science' | 'Math' | 'English' | 'Reading';
  definition: string;
  teas_context: string; // How it appears on the exam
  related_terms: string[];
}

export const wikiData: WikiTerm[] = [
  // SCIENCE (High Volume)
  { 
    slug: "mitochondria", 
    term: "Mitochondria", 
    ...
    related_terms: ["ATP", "Cellular Respiration", "Eukaryotic Cell"]
  },
  { 
    slug: "mitosis", 
    term: "Mitosis", 
...
export function getWikiBySlug(slug: string): WikiTerm | undefined {
  return wikiData.find((item) => item.slug === slug);
}

export function getAllWikiSlugs(): string[] {
  return wikiData.map((item) => item.slug);
}